...
(*ENDIF-OCAML*)
(*F#
let to_string (s:stream) =
  // TODO: this code is quite possibly  wrong.
  let chars = Array.create (s.buf.Length) ((char)0) in
  let _ = s.buf.CopyTo(0,chars,0,s.buf.Length) in
  List.iter (fun (pos, b) -> chars.[pos] <- (char) b) !(s.patches);
  chars.ToString()
F#*)
...
(*
  let module E = struct
    ...
    *)
    ...
    let u32 i =
      u16 (Int32.to_int (Int32.logand i 0xffffl));
             u16 (Int32.to_int (Int32.shift_right i 16)) in
    let u64 i =
      u32 (Int64.to_int32 (Int64.logand i 0xffffffffL));
             u32 (Int64.to_int32 (Int64.shift_right i 32)) in
    ...
    let rec vs64 i =
      let b = Int64.to_int (Int64.logand i 0x7fL) in
      if -64L <= i && i < 64L then u8 b
      else (u8 (b lor 0x80); vs64 (Int64.shift_right i 7)) in

    let vu1 i = vu64 (Int64.logand (Int64.of_int i) 1L) in 
    let vu32 i = vu64 (Int64.logand (Int64.of_int32 i) 0xffffffffL) in
    let vs7 i = vs64 (Int64.of_int i) in
    let vs32 i = vs64 (Int64.of_int32 i) in
    let f32 x = u32 (F32.to_bits x) in
    let f64 x = u64 (F64.to_bits x) in

    let len i =
      if Int32.to_int (Int32.of_int i) <> i then
        Code.error Source.no_region
          "cannot encode length with more than 32 bit";
      vu32 (Int32.of_int i) in 

    let bool b = vu1 (if b then 1 else 0) in
    let string bs = len (String.length bs); put_string s bs in
    let name n = string (Utf8.encode n) in
    let list f xs = List.iter f xs in
    let opt f xo = Lib.Option.app f xo in
    let vec f xs = len (List.length xs); list f xs in

    let gap32 () = let p = pos s in u32 0l; u8 0; p in
    let patch_gap32 p n =
      assert (n <= 0x0fffffff); (* Strings cannot excess 2G anyway *)
      let lsb i = Char.chr (i land 0xff) in
      patch s p (lsb (n lor 0x80));
      patch s (p + 1) (lsb ((n lsr 7) lor 0x80));
      patch s (p + 2) (lsb ((n lsr 14) lor 0x80));
    ...
    let stack_type = function
      ...
      | _ ->
        Code.error Source.no_region
          "cannot encode stack type with arity > 1 (yet)" in

    let func_type = function
      | FuncType (ins, out) -> vs7 (-0x20); vec value_type ins; vec value_type out in

    let limits vu {min=min; max=max} =
      bool (max <> None); vu min; opt vu max in

    let table_type = function
      | TableType (lim, t) -> elem_type t; limits vu32 lim in

    let memory_type = function
      | MemoryType lim -> limits vu32 lim in
    ...
    let global_type = function
      | GlobalType (t, mut) -> value_type t; mutability mut in
    ...
    let memop {align=align; offset=offset; ty= _ ; sz = _} = vu32 (Int32.of_int align); vu32 offset in

    let var x = vu32 x.it in
    ...
    let rec instr e =
      ...
      | Load ({ty = I32Type; sz = None; align = _; offset = _} as mo) -> op 0x28; memop mo
      | Load ({ty = I64Type; sz = None; align = _; offset = _} as mo) -> op 0x29; memop mo
      | Load ({ty = F32Type; sz = None; align = _; offset = _} as mo) -> op 0x2a; memop mo
      | Load ({ty = F64Type; sz = None; align = _; offset = _} as mo) -> op 0x2b; memop mo
      | Load ({ty = I32Type; sz = Some (Mem8, SX); align = _; offset = _} as mo) ->
        op 0x2c; memop mo
      | Load ({ty = I32Type; sz = Some (Mem8, ZX); align = _; offset = _} as mo) ->
        op 0x2d; memop mo
      | Load ({ty = I32Type; sz = Some (Mem16, SX); align = _; offset = _} as mo) ->
        op 0x2e; memop mo
      | Load ({ty = I32Type; sz = Some (Mem16, ZX); align = _; offset = _} as mo) ->
        op 0x2f; memop mo
      | Load {ty = I32Type; sz = Some (Mem32, _); align = _; offset = _} ->
        assert false
      | Load ({ty = I64Type; sz = Some (Mem8, SX); align = _; offset = _} as mo) ->
        op 0x30; memop mo
      | Load ({ty = I64Type; sz = Some (Mem8, ZX); align = _; offset = _} as mo) ->
        op 0x31; memop mo
      | Load ({ty = I64Type; sz = Some (Mem16, SX); align = _; offset = _} as mo) ->
        op 0x32; memop mo
      | Load ({ty = I64Type; sz = Some (Mem16, ZX); align = _; offset = _} as mo) ->
        op 0x33; memop mo
      | Load ({ty = I64Type; sz = Some (Mem32, SX);align = _; offset = _} as mo) ->
        op 0x34; memop mo
      | Load ({ty = I64Type; sz = Some (Mem32, ZX); align = _; offset = _} as mo) ->
        op 0x35; memop mo
      | Load {ty = F32Type | F64Type; sz = Some _; align = _; offset = _} ->
        assert false

      | Store ({ty = I32Type; sz = None; align = _; offset = _} as mo) -> op 0x36; memop mo
      | Store ({ty = I64Type; sz = None; align = _; offset = _} as mo) -> op 0x37; memop mo
      | Store ({ty = F32Type; sz = None; align = _; offset = _} as mo) -> op 0x38; memop mo
      | Store ({ty = F64Type; sz = None; align = _; offset = _} as mo) -> op 0x39; memop mo
      | Store ({ty = I32Type; sz = Some Mem8; align = _; offset = _} as mo) -> op 0x3a; memop mo
      | Store ({ty = I32Type; sz = Some Mem16; align = _; offset = _} as mo) -> op 0x3b; memop mo
      | Store {ty = I32Type; sz = Some Mem32; align = _; offset = _} -> assert false
      | Store ({ty = I64Type; sz = Some Mem8; align = _; offset = _} as mo) -> op 0x3c; memop mo
      | Store ({ty = I64Type; sz = Some Mem16; align = _; offset = _} as mo) -> op 0x3d; memop mo
      | Store ({ty = I64Type; sz = Some Mem32; align = _; offset = _} as mo) -> op 0x3e; memop mo
      | Store {ty = F32Type | F64Type; sz = Some _; align = _; offset = _} -> assert false

      | CurrentMemory -> op 0x3f; u8 0x00
      | GrowMemory -> op 0x40; u8 0x00

      | Const {it = I32 c; at= _} -> op 0x41; vs32 c
      | Const {it = I64 c; at= _} -> op 0x42; vs64 c
      | Const {it = F32 c; at= _} -> op 0x43; f32 c
      | Const {it = F64 c; at= _} -> op 0x44; f64 c
      ...
      | Convert (F64 F64Op.PromoteF32) -> op 0xbb
      | Convert (F64 F64Op.DemoteF64) -> assert false
      | Convert (F64 F64Op.ReinterpretInt) -> op 0xbf in
    

    let ``const`` c =
      list instr c.it; end_ () in
    ...
    (* Type section *)
    let type_ t = func_type t.it in

    let type_section ts =
      section 1 (vec type_) ts (ts <> []) in
    ...
    let import_desc d =
      ...
      | TableImport t -> u8 0x01; table_type t
      | MemoryImport t -> u8 0x02; memory_type t
      | GlobalImport t -> u8 0x03; global_type t in

    let import im =
      let {module_name=module_name; item_name = item_name; idesc = idesc} = im.it in
      name module_name; name item_name; import_desc idesc in

    let import_section ims =
      section 2 (vec import) ims (ims <> []) in

    (* Function section *)
    let func f = var f.it.ftype in

    let func_section fs =
      section 3 (vec func) fs (fs <> []) in
    ...
    let table_section tabs =
      section 4 (vec table) tabs (tabs <> []) in

    (* Memory section *)
    let memory mem =
      let {mtype=mtype} = mem.it in
      memory_type mtype in

    let memory_section mems =
      section 5 (vec memory) mems (mems <> []) in

    (* Global section *)
    let ``global`` g =
      let {gtype=gtype; value=value} = g.it in
      global_type gtype; ``const`` value in

    let global_section gs =
      section 6 (vec ``global``) gs (gs <> []) in
    ...
    let export_desc d =
      ...
      | TableExport x -> u8 1; var x
      | MemoryExport x -> u8 2; var x
      | GlobalExport x -> u8 3; var x in

    let export ex =
      let {name = n; edesc = edesc} = ex.it in
      name n; export_desc edesc in

    let export_section exs =
      section 7 (vec export) exs (exs <> []) in

    (* Start section *)
    let start_section xo =
      section 8 (opt var) xo (xo <> None) in
    ...
    let compress ts =
      let combine t = function
        | (t', n) :: ts when t = t' -> (t, n + 1) :: ts
        | ts -> (t, 1) :: ts
      in List.fold_right combine ts [] in

    let local (t, n) = len n; value_type t in

    let code f =
      let {locals=locals; body=body; ftype=ftype} = f.it in
      let g = gap32 () in
      let p = pos s in
      vec local (compress locals);
      list instr body;
      end_ ();
      patch_gap32 g (pos s - p) in

    let code_section fs =
      section 10 (vec code) fs (fs <> []) in

    (* Element section *)
    let segment dat seg =
      let {index=index; offset=offset; init=init} = seg.it in
      var index; ``const`` offset; dat init in

    let table_segment seg =
      segment (vec var) seg in

    let elem_section elems =
      section 9 (vec table_segment) elems (elems <> []) in

    (* Data section *)
    let memory_segment seg =
      segment string seg in

    let data_section data =
      section 11 (vec memory_segment) data (data <> []) in
