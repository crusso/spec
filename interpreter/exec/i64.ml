...
(*F#
  //crusso: we inline the functor application
  module Rep = struct
    type t = System.Int64
    ...
    let one : t = 1L
    let minus_one : t = -1L
    let max_int : t = System.Int64.MaxValue
    let min_int : t = System.Int64.MinValue
    
    let neg(i:t) = -i
    let add(i:t)(j:t) = i+j
    let sub(i:t)(j:t) = i-j
    let mul(i:t)(j:t) = i*j
    let div(i:t)(j:t) = i / j  (* raises Division_by_zero *) //TBR
    let rem(i:t)(j:t) = i % j (* raises Division_by_zero *)  //TBR
    
    let logand(i:t)(j:t) = i &&& j
    let lognot(i:t) = ~~~i
    let logor(i:t)(j:t) = i ||| j
    let logxor (i:t)(j:t) = i ^^^ j
    let shift_left(i:t)(j:int) = i <<< j //TBR
    let shift_right(i:t)(j:int) = i >>> j //TBR
    let shift_right_logical(i:t)(j:int32) = (int64) ((uint64) i >>>  j) //TBT
    let of_int (i:int) : t = (int64) i
    let to_int (i:t) : int = (int32) i
    let to_string  (i:t) =  i.ToString()
  ...
  let of_int_u i = and_ (Rep.of_int i) (or_ (shl (Rep.of_int System.Int32.MaxValue) one) one) //TBR - cf i32
  ...
  module Char = 
  struct
     let code (c:char) = (int32) c // TBR
  end

  let dec_digit = function
    //| '0' .. '9' as c -> Char.code c - Char.code '0'
    | c when c >= '0' && c <= '9' -> Char.code c - Char.code '0'
  ...
  let hex_digit = function
    ...
    |  c when c >= 'a' && c <= 'f'  ->  0xa + Char.code c - Char.code 'a'
    //| 'A' .. 'F' as c ->  0xa + Char.code c - Char.code 'A'
    |  c when c >= 'A' && c <= 'F' ->  0xa + Char.code c - Char.code 'A'
    | _ ->  failwith "of_string"
